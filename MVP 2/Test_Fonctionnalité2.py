...
def test_longueur_nom_fonctions(Nom_dev):
    """teste la fonction longueur_nom_fonctions
    :param: le nom du développeur """
    assert longueur_nom_fonctions(les_filepath(Nom_dev,CandidatA)[0]) == [[8,12,19,40,48,42,48,46,20],21]

def test_longueur_nom_variables(Nom_dev):
    """teste la fonction longueur_nom_varaiables
    :param: le nom du développeur """
    assert longueur_nom_variables(les_filepath(Nom_dev,CandidatA)[0]) == [[15, 4, 10, 8, 14, 8, 12, 5], 0.76]

def test_count_imbrication(Nom_dev):
    """teste la fonction count_imbrication
    :param: le nom du développeur"""
    assert count_imbrications(les_filepath(Nom_dev,CandidatA)[0]) == 0.5

def test_nombre_duplications(Nom_dev):
     """teste la fonction count_duplications
     :param: le nom du développeur et le nom du candidat dont on analyse le code"""
     assert count_duplications(les_filepath(Nom_dev,CandidatA)[0]) == 0
...
def test_fonctions_comprehensibles(Nom_dev):
    """teste la fonction fonctions_comprehensibles
    :param: le nom du développeur """
    assert type(fonctions_comprehensibles(les_filepath(Nom_dev,CandidatA)[0])) == float


def test_variables_comprehensibles(Nom_dev):
    """teste la fonction varaibles_comprehensibles
    :param: le nom du développeur """
    assert type(variables_comprehensibles(les_filepath(Nom_dev,CandidatA)[0])) == float
